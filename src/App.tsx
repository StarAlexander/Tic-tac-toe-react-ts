...
const Square:React.FC<SquareProps> = ({value,onClick,fraction}) => {
  return <button onClick={onClick} className={`${fraction==20?'w-20 h-20':fraction==15?'w-15 h-15':'w-12 h-12'} border border-cyan-200`}>{value}</button>
}
...
const App:React.FC = () => {
  return (
    <div className="dark:bg-black flex flex-col items-center justify-center dark:text-white w-screen h-screen text-center">
      <Board/>
    </div>
  )
}
